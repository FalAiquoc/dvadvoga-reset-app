...
['/:token', '/resetarsenha/:token'].forEach(route => {
...
fastify.get('/api/check-2fa/:token', async (request, reply) => {
      ...
      if (!row) return reply.status(404).send({ error: 'Token invalido' });

                let user = db.prepare('SELECT totp_secret FROM users WHERE email = ?').get(row.email);
      if (user && user.totp_secret) {
                ...
                return { has2fa: false, qrImage, secret };
      }
});
...
fastify.post('/api/reset', async (request, reply) => {
      ...
      const row = db.prepare('SELECT email, expires_at FROM tokens WHERE token = ?').get(token);
                 ...
                 if (!row || Date.now() > row.expires_at) {
                           return reply.status(404).send({ error: 'Token invalido ou expirado.' });
                 }
      ...
      if (user && user.totp_secret) {
                ...
                if (!isValid) {
                              return reply.status(400).send({ error: 'Codigo de seguranca invalido.' });
                }
      }
